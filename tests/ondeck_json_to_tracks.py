...
def main(ctx, dbname, dbuser):
    ...
    if ctx.invoked_subcommand is None:
        ...
        with sessionmaker() as session:
            active_tracks = {}
            done_tracks = []
            fname = "/Users/ericfultz/Documents/pops/TNC/tnc-edge-service/tests/data/20231205T212500Z_cam1_ondeck.json"
            import json
...
def archive(ctx):
    ...
    with session:
        res = session.execute(
            sa.text(
                "select ondeckdata.video_uri, ondeckdata.cocoannotations_uri from ondeckdata \

...
EXTHDPATH="/Volumes/Expansion"

cd $EXTHDPATH || exit 1

# exit 0

for i in brancol_20231015T134500Z_cam1.avi.enc brancol_20231015T135000Z_cam1.avi.enc ; do 
...
done | parallel -v -r --eta --jobs 6 --colsep " " 'flock '$SCRIPTDIR'/usb.lock cp {1} '$SCRIPTDIR'/{1} && aws s3 --profile AWSAdministratorAccess-867800856651 cp '$SCRIPTDIR'/{1} "s3://dp.riskedge.fish/TNC EDGE Trip Video Files/{2}/alt_hd_upload/{3}" && flock '$SCRIPTDIR'/done.lock echo {1} >> '$SCRIPTDIR'/done.txt && rm '$SCRIPTDIR'/{1}' 2>&1 
...
for i in *.avi *.mkv ; do 
  ...
  dtname="${bname#*_}"
  echo "$bname" "${boatname^}" "$dtname"
done | parallel -v -r --eta --jobs 4 --colsep " " 'flock '$SCRIPTDIR'/usb.lock cp {1} '$SCRIPTDIR'/{1} && aws s3 --profile AWSAdministratorAccess-867800856651 cp '$SCRIPTDIR'/{1} "s3://dp.riskedge.fish/TNC EDGE Trip Video Files/{2}/alt_hd_upload/{3}" && flock '$SCRIPTDIR'/done.lock echo {1} >> '$SCRIPTDIR'/done.txt && rm '$SCRIPTDIR'/{1}' 2>&1 

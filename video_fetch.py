    ...
    try:
        ...
        for date_dir in date_dirs:
            ...
            for hour_dir in hour_dirs:
                vid_files = [
                    x for x in hour_dir.iterdir() if x.is_file() and re.match(".*-(\d+)\.", x.name)
                ]
...
def is_gpg(f: Path, passphrase_file: str):
    cmd = (
        "cat %s | gpg --pinentry-mode loopback --passphrase-fd 0 \
                    --list-packets %s "
        % (passphrase_file, str(f.absolute()))
    )
...
def video_fetch(
    cpool: SimpleConnectionPool,
    thalos_dir: Path,
    output_dir: Path,
    passphrase_file: str,
    thalos_video_suffix: str,
):
    for cameradir in filter(lambda x: x.is_dir(), thalos_dir.iterdir()):
        ...
        try:
            with conn.cursor() as cur:
                for vid_file in depth_first_video_files(cameradir):
                    ...
                    vid_file_done_alt_str = (
                        vid_file_absolute_str[0 : -len(".done")]
                        if vid_file_absolute_str.endswith(".done")
                        else vid_file_absolute_str + ".done"
                    )

                    start_datetime: datetime = datetime.strptime(
                        vid_file.name[0 : len("20-07-2023-22-20")], "%d-%m-%Y-%H-%M"
                    )
                    ...
                    cur.execute(
                        "select original_path, last_modified from video_files where original_path in (%s, %s);",
                        (
                            vid_file_absolute_str,
                            vid_file_done_alt_str,
                        ),
                    )
                    rows = list(cur)
                    if len(rows) == 0:
                        ...
                        cur.execute(
                            "update video_files set last_modified = %s where original_path in (%s, %s);",
        ...
        for new_vid in new_vids:
            ...
            new_vid_done_alt_str = (
                new_vid_absolute_str[0 : -len(".done")]
                if new_vid_absolute_str.endswith(".done")
                else new_vid_absolute_str + ".done"
            )
            ...
            try:
                with conn.cursor() as cur:
                    cur.execute(
                        "select original_path, last_modified, start_datetime, \
                                decrypted_path, decrypted_datetime, stdout, stderr \
                                from video_files where original_path in ( %s, %s );",
                        (
                    ...
                    if is_gpg(new_vid, passphrase_file):
                        cmd = (
                            ...
                            --decrypt --output %s %s "
                            % (passphrase_file, str(output_file.absolute()), new_vid_absolute_str)
                        )
                    else:
                        cmd = "cp %s %s" % (new_vid_absolute_str, str(output_file.absolute()))
                    p = subprocess.run(cmd, shell=True, capture_output=True, text=True)
                    if p.returncode == 0:
                        cur.execute(
                            "update video_files set decrypted_path = %s, \
...
def main(
    dbname, dbuser, thalos_video_dir, output_dir, passphrase_file, thalos_video_suffix, print_latest
):
    ...
    schedule.every(1).seconds.do(
        runonce, cpool, thalos_dir, output_dir, passphrase_file, thalos_video_suffix
    )

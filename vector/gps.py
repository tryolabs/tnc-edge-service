...
class GpsVector:
    ...
    def execute(self, expected_timedelta, gpsDataSelect):
        ...
        result = Test(
            name="gps test from %s to %s" % (datetime_from, datetime_to),
            vector=self.rv,
        ...
        gpsDatas = (
            self.session.query(GpsData)
            .filter(GpsData.datetime > datetime_from)
            .filter(GpsData.datetime < datetime_to)
        )
...
def intersects(seg1, seg2):
    ...
    isectx = (slope1 * seg1[0][0] - seg1[0][1] - slope2 * seg2[0][0] + seg2[0][1]) / (
        slope1 - slope2
    )
    # print("isectx", isectx)

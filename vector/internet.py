...
class InternetVector:
    ...
    def execute(self, expected_timedelta: timedelta):
        ...
        result = Test(
            name="internet test at %s" % (datetime_to.strftime("%Y-%m-%d %H:%M:%SZ")),
            vector=self.rv,
        )
        ...
        f = filter(
            lambda data: data.returncode != 0 or data.packetloss and data.packetloss > 33.4, datas
        )
...
def traceroute(ip):
    cmd = (
        "traceroute -m 12 %s | grep -E '^\s*[0-9][0-9]*\s*' | grep -v '\* \* \*' | awk '{{print $2 \" \" $3}}' "
        % (ip)
    )

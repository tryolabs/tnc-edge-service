...
class ElogTimeGapsVector:
    ...
    def execute(self, expected_timedelta: timedelta):
        ...
        last_departure_res = self.session.execute(
            sa.text("""select max(datetime) last_departure from port_departures;""")
        )

        last_departure: datetime = last_departure_res.first()[0]

        result = Test(
            name="elog time gap vector at %s" % (datetime_to.strftime("%Y-%m-%d %H:%M:%SZ")),
            vector=self.rv,
        )
